<form
    action="{{ isset($exchangeRate) ? route('exchange-rates.update', $exchangeRate->id) : route('exchange-rates.store') }}"
    method="POST" data-loading-form
        ...
        <div>
            ...
            @if ($field === 'source_currency')
                <input type="text" id="{{ $field }}" name="{{ $field }}"
                    class="mt-2 block w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 shadow-sm"
                    value="BOB" readonly />
            @elseif ($field === 'rate')
                <input type="number" step="0.00000001" id="{{ $field }}" name="{{ $field }}"
                    ...
                    value="{{ old($field, isset($exchangeRate) ? $exchangeRate->$field : '') }}" />
            @else
                <input type="text" id="{{ $field }}" name="{{ $field }}"
                    class="mt-2 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    value="{{ old($field, isset($exchangeRate) ? $exchangeRate->$field : '') }}" />
    ...
    <div class="flex items-center justify-between mt-4">
        ...
        <label class="relative inline-flex items-center cursor-pointer">
            ...
            <input type="checkbox" name="active" class="sr-only peer" value="1"
                {{ old('active', isset($exchangeRate) ? $exchangeRate->active : true) ? 'checked' : '' }}>
            <div
                class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600">
            </div>
            <span class="ml-3 text-sm font-medium text-gray-900">
                {{ old('active', isset($exchangeRate) ? $exchangeRate->active : true) ? 'Activo' : 'Inactivo' }}
            </span>
        </label>
    </div>
    ...
    <button type="submit"
        ...
        data-loading-text="{{ isset($exchangeRate) ? 'Actualizando...' : 'Creando...' }}"
        data-loading-classes="from-gray-400 to-gray-500">
        <span data-button-text> {{ isset($exchangeRate) ? 'Actualizar tasa' : 'Crear tasa de cambio' }}</span>
