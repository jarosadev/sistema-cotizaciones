    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white rounded-xl shadow-sm p-3 mb-6 border border-gray-200">
        <div class="flex items-center gap-6 sm:flex-row flex-col">
            <h2 class="text-xl font-black text-gray-800">
                <span class="text-[#0B628D]">Ciudades</span>
            </h2>
        ...
        <div class="flex space-x-2 sm:flex-row flex-col max-sm:gap-2">
            <a href="{{ route('cities.trashed') }}" 
               class="flex items-center justify-center px-4 py-2 bg-[#0b8d41] hover:bg-[#588498] text-white text-sm font-medium rounded-lg transition-colors duration-200 shadow-sm text-center max-sm:w-full">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody id="citiesTableBody" class="bg-white divide-y divide-gray-200">
                    ...
                    @else
                        ...
                        <tr class="city-row hover:bg-gray-50 transition-colors duration-150"
                            data-name="{{ strtolower($city->name) }}"
                            @if(isset($city->country)) 
                                data-country="{{ strtolower($city->country->name) }}"
                            @endif>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        if (!hasResults && cityRows.length > 0) {
